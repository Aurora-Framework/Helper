...
class Str
{
    ...
    public static function limitWords($string, $limit = 15, $end = "")
    {
        return preg_replace('/((\w+\W*|| [\p{L}]+\W*){'.($limit-1).'}(\w+))(.*)/', '${1}', $string).$end;
    }

    public static function limitCharacters($string, $limit = 15, $end = "")
    {
        return preg_match('(\S{'.$limit.'})', $string).$end;
    }

    public static function censorWords($string, $disallowed = null, $replacement = "")
    {
        return str_replace($disallowed, $replacement, $string);
    }
